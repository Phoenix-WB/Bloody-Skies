...
public class PlayerEvents {
	...
	public static void playerSanguineScapesDetection(LivingEvent.LivingUpdateEvent event) {
		...
		if (biomeskycolour == 12132898 && world.getDayTime() % 24000 < 13000) {
	...
	@SubscribeEvent
	public static void bloodClothPlacedOnPedestal(PlayerInteractEvent.RightClickBlock event) {
		ItemStack bloodCloth = event.getItemStack();
		BlockPos blockPos = event.getHitVec().getBlockPos();
		BlockState bloodAltar = BlockInit.BLOOD_ALTAR.get().defaultBlockState();
		Boolean isBloodCloth = bloodCloth.getItem() == ItemInit.BLOOD_CLOTH.get();
		Boolean isPedestal = event.getWorld().getBlockState(blockPos) == BlockInit.PEDESTAL.get().defaultBlockState();
		if (isPedestal && isBloodCloth) {
			event.getWorld().setBlockAndUpdate(blockPos, bloodAltar);
			bloodCloth.shrink(1);
		}
	}
}